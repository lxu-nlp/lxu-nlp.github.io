---
layout: post
title: "Works on AI for Research"
date: 2026-03-11
categories: [LLM]
tags: [nlp, LLM, AI4S]
math: true
---

## Synthesis and QA

**OpenResearcher: Unleashing AI for Accelerated Scientific Research**. Zheng et al. EMNLP Demo'24\
<https://aclanthology.org/2024.emnlp-demo.22>

**OpenScholar: Synthesizing Scientific Literature with Retrieval-augmented LMs**. Asai et al. 2024\
<https://arxiv.org/abs/2411.14199>

**Language agents achieve superhuman synthesis of scientific knowledge**. Skarlinski et al. 2024\
QA.\
<https://arxiv.org/abs/2409.13740>

**ArxivDIGESTables: Synthesizing Scientific Literature into Tables using Language Models**. Newman et al. EMNLP'24\
Synthesize literature review tables.\
<https://aclanthology.org/2024.emnlp-main.538/>

## Discovery

**Can LLMs Generate Novel Research Ideas? A Large-Scale Human Study with 100+ NLP Researchers**. Si et al. ICLR'25\
<https://openreview.net/forum?id=M23dTGWCZy>

**DiscoveryBench: Towards Data-Driven Discovery with Large Language Models**. Majumder et al. ICLR'25\
<https://openreview.net/forum?id=vyflgpwfJW>

**AI Can Learn Scientific Taste**. Tong et al. 2026\
SciJudgeBench: impact prediction as pairwise preference, by arxiv citation and ICLR review score.\
<https://arxiv.org/abs/2603.14473>

## Code Generation

**ScienceAgentBench: Toward Rigorous Assessment of Language Agents for Data-Driven Scientific Discovery**. Chen et al. ICLR'25\
Given task, data, domain knowledge -> evaluation results from code gen.\
<https://openreview.net/forum?id=6z4YKr0GK6>

## Full Stack

**The AI Scientist: Towards Fully Automated Open-Ended Scientific Discovery**. Lu et al. 2024\
Designed workflow.\
Evaluation: automated review on generated papers.\
<https://arxiv.org/abs/2408.06292>

**AI-Researcher: Autonomous Scientific Innovation**. Tang et al. NIPS'25\
Designed workflow.\
Evaluation: 1) LLM scoring by designed rubrics; 2) automated review score on generated papers.\
<https://openreview.net/pdf/a1c63cdd0495de94664b1513f7d95a3aedcb483a.pdf>

**AstaBench: Rigorous Benchmarking of AI Agents with a Scientific Research Suite**. Bragg et al. 2025\
Benchmark suite.\
<https://arxiv.org/abs/2510.21652>
